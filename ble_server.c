...
static uint8_t adv_data[] = {
    ...
    // Name
    0x0D, BLUETOOTH_DATA_TYPE_COMPLETE_LOCAL_NAME, 'P', 'M', '2', '.', '5', ' ', 'L', 'o', 'g', 'g', 'e', 'r',
...
void ble_server_notify_pm25(uint16_t pm25_value) {
    if (server_con_handle == HCI_CON_HANDLE_INVALID) {
        // No client connected, skip notification
        return;
    }

    // Prepare a 2-byte buffer for the uint16_t value (Little Endian)
    uint8_t buffer[2];
    little_endian_store_16(buffer, 0, pm25_value);
    
    // Check if the client has enabled notifications for this characteristic
    // ATT_CHARACTERISTIC_0xAAA4_01_VALUE_HANDLE is generated by the GATT compiler
    att_server_notify(server_con_handle, ATT_CHARACTERISTIC_0xAAA4_01_VALUE_HANDLE, buffer, sizeof(buffer));
    printf("BLE Notified PM2.5: %u\n", pm25_value);
}
...
void ble_server_notify_gas(uint32_t no2, uint32_t c2h5oh, uint32_t voc, uint32_t co) {
    ...
    // Create a 16-byte buffer (4 integers * 4 bytes each)
    uint8_t buffer[16];

    // Pack data in Little Endian format (standard for BLE)
    little_endian_store_32(buffer, 0, no2);
    little_endian_store_32(buffer, 4, c2h5oh);
    little_endian_store_32(buffer, 8, voc);
    little_endian_store_32(buffer, 12, co);

    // Send notification to the new Characteristic Handle (0xAAA5)
    // Note: The handle name is auto-generated by the build system based on the GATT file
    att_server_notify(server_con_handle, ATT_CHARACTERISTIC_0xAAA5_01_VALUE_HANDLE, buffer, sizeof(buffer));
    
    // Optional: Print debug info (careful not to spam too much)
    printf("BLE Notified Gas: %lu, %lu, %lu, %lu\n", no2, c2h5oh, voc, co);
}

void ble_server_notify_env(uint16_t co2, int32_t temp, int32_t hum) {
    if (ble_server_get_con_handle() == HCI_CON_HANDLE_INVALID) return;

    // Payload: 2 + 4 + 4 = 10 bytes
    uint8_t buffer[10];
    
    little_endian_store_16(buffer, 0, co2);
    little_endian_store_32(buffer, 2, (uint32_t)temp); // Cast to uint32 for transport
    little_endian_store_32(buffer, 6, (uint32_t)hum);

    // Notify characteristic AAA6 (Check your generated .h for the exact handle name)
    att_server_notify(ble_server_get_con_handle(), ATT_CHARACTERISTIC_0xAAA6_01_VALUE_HANDLE, buffer, sizeof(buffer));
    
    // Optional Debug
    // printf("BLE Notified Env: CO2=%u\n", co2);
}
...
static void stream_timer_handler(btstack_timer_source_t *ts) {
    ...
    if (bytes_read > 0) {
        ...
        btstack_run_loop_set_timer(ts, 15); // 15ms delay
...
static int att_write_callback(hci_con_handle_t connection_handle, uint16_t att_handle, uint16_t transaction_mode, uint16_t offset, uint8_t *buffer, uint16_t buffer_size) {
    ...
    if (att_handle == ATT_CHARACTERISTIC_0xAAA1_01_VALUE_HANDLE) { 
        ...
        datetime_t t; // Now defined by pico/util/datetime.h
        ...
        // Convert datetime_t (t) to C standard struct tm (tm_struct)
            struct tm tm_struct = {0}; // Initialize to zero
            
            // Direct conversion of datetime_t fields to struct tm fields
            // Note: tm_year is years since 1900, tm_mon is 0-11
            tm_struct.tm_year = t.year - 1900;
            tm_struct.tm_mon  = t.month - 1;
            tm_struct.tm_mday = t.day;
            tm_struct.tm_hour = t.hour;
            tm_struct.tm_min  = t.min;
            tm_struct.tm_sec  = t.sec;
            
            // Set time using the unified AON Timer API
            bool success = aon_timer_set_time_calendar(&tm_struct);
