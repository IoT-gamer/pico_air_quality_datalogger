...
/**
 * @brief Sends a BLE notification with the current PM2.5 value (2 bytes).
 * @param pm25_value The PM2.5 reading (ug/m3) to notify.
 */
void ble_server_notify_pm25(uint16_t pm25_value);

/**
 * @brief Sends a BLE notification with the environmental sensor values.
 * Payload is 10 bytes: uint16_t (CO2) + int32_t (temp) + int32_t (hum).
 */
void ble_server_notify_env(uint16_t co2, int32_t temp, int32_t hum);
...
/**
 * @brief Sends a BLE notification with the gas sensor values.
 * Payload is 16 bytes (4 x uint32_t).
 */
void ble_server_notify_gas(uint32_t no2, uint32_t c2h5oh, uint32_t voc, uint32_t co);
